...
export const generate_card = async (data: DroidScoreScraped[]): Promise<Buffer> =>  {
	...
	const avatar = await loadImage(`https://osudroid.moe/user/avatar/${user.id}.png`)
	...
	const color = await getAverageColor(`https://osudroid.moe/user/avatar/${user.id}.png`)
	...
	ctx.fillText(`${user.pp.toLocaleString("en-US")}`, 30, 350)
	if (user.rank.pp < 150 && user.rank.pp > 50) {
		ctx.fillStyle  = "rgb(255, 238, 153)"
	} else if (user.rank.pp < 50) {
		ctx.fillStyle = "rgb(154, 223, 234)"
	}

	ctx.fillText(`#${user.rank.pp.toLocaleString("en-US")}`, 190, 350)
	ctx.fillStyle = "rgb(255,255,255,255)"
	...
	ctx.fillStyle = "rgb(255,255,255,255)"
	ctx.textAlign = "right"
	ctx.fillText(user.score.toLocaleString(), 480, 420)
