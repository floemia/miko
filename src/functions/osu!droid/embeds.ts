...
const score = async (recent: DroidScoreScraped) => {
    ...
    } else { embed.setAuthor({name: `${recent.beatmap.artist} - ${recent.beatmap.title} [${recent.beatmap.version}] ${recent.performance.stars_dr? `${recent.performance.stars_dr.toFixed(2)} ⭐`: ''} +${mods.str} ${mods.speed != 1 ? `(${mods.speed.toFixed(2)}x)` : ``} `, iconURL: recent.user.avatar_url, url: `https://osu.ppy.sh/beatmapsets/${recent.beatmap.beatmapsetID}#osu/${recent.beatmap.beatmapID}`})}
    ...
    embed.setTimestamp(recent.timestamp)
