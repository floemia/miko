...
export const command: Command = {
	data: new SlashCommandBuilder()
		...
		.addStringOption(opt => opt
			.setName("server")
			.setDescription("The server to show the play from. Defaults to iBancho.").setDescriptionLocalization("es-ES", "El servidor desde el cual mostrar la play. Por defecto, iBancho.")
			.addChoices(
	...
	async execute(client, interaction) {
		...
		if(!recents.length) return await interaction.editReply({
				embeds: [utils.embeds.error({ description: response.command.recent.no_scores(user), interaction: interaction, spanish: spanish })]
			})
